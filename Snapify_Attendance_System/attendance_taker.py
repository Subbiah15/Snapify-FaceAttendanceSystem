...
# Create a connection to the database and initialize schema
db_manager.init_db()
...
class Face_Recognizer:
    ...
    def process(self, stream):
        # original loop-based processing (used by desktop script)
        ...
        if self.get_face_database():
            while stream.isOpened():
                ...
                annotated = self._handle_frame(img_rd)
                ...
                cv2.imshow("Snapify - Face Recognition System", annotated)

                logging.debug("Frame ends\n\n")

    def _handle_frame(self, img_rd):
        """Internal helper; perform recognition on a single frame and return
        the annotated image.  Most of the logic from the original loop has
        been refactored here so that mobile code can call it directly."""
    ...
    # new helper for mobile integration
    def process_frame(self, img_rd):
        """Public method suitable for feeding one frame at a time from an
        external source (e.g. Kivy camera loop). The first call will load
        the face database automatically."""
